...
"Wisdom" may be a bit overwrought here, but there's ways to use the language we have to effect/cover many of the cases that DUs solve outright. The trouble is, as "idioms" they are often subtle and easy to misuse—the same way a human-language learner might trip over difficult idioms in their second language. In a sense, DUs are a language structure that—again [analagous](#foot-1)) to human langage—we might say, "has no equivalent expression in C#."

Nonetheless, the _idioms_ that are available to us allow for logical equvalence, if not equivalence in semantic or resource-usage.
